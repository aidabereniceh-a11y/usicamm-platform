...
export default function SimuladorUsicamm2026() {
  ...
  function iniciarExamen() {
    ...
    const filtradas = preguntasUsicamm.filter(
      (pregunta) => pregunta.categoria === categoriaSeleccionada
    );
    const mezcladas = [...filtradas].sort(() => Math.random() - 0.5);
  ...
  // TIMER
  useEffect(() => {
    if (examenIniciado && tiempo > 0 && !finalizado) {
  ...
  // VALIDACIÓN
  if (examenIniciado && preguntas.length > 0 && !preguntaActual) {
    return null;
  }
  ...
  if (!examenIniciado) {
    return (
      ...
      >
        <h1 style={{ color: "#1e3a8a" }}>Simulador USICAMM 2026</h1>
          ...
          {categorias.map((categoria) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                backgroundColor:
                  categoriaSeleccionada === categoria ? "#1e3a8a" : "#2563eb",
                color: "white",
                fontSize: "16px",
              }}
            >
  ...
  return (
    ...
    >
      ...
      <h3 style={{ textAlign: "center", marginTop: "10px", color: "#2563eb" }}>
